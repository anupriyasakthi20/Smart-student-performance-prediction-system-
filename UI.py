...
def validate_internal_marks(event=None):
    ...
    try:
        number = float(value)
        if number < 0 or number > 100:
            internal_error.config(text="Must be 0 - 100")
        else:
            internal_error.config(text="")
    except ValueError:
...
def predict_performance():
    try:
        ...
        if attendance_entry.get().strip() == "":
            ...
            return
        if study_hours_entry.get().strip() == "":
            messagebox.showerror(
                "Missing Input",
            ...
            return
        if internal_marks_entry.get().strip() == "":
            messagebox.showerror(
                "Missing Input",
                "Please enter Internal Marks."
            )
            internal_marks_entry.focus()
            return
        if assignment_entry.get().strip() == "":
            messagebox.showerror(
                "Missing Input",
            ...
            return
        if previous_score_entry.get().strip() == "":
            messagebox.showerror(
                "Missing Input",
                "Please enter Previous Score."
            )
            previous_score_entry.focus()
            return
        ...
        if predicted_score >= 75:
            risk_level = "Low Risk"
            recommendation = (
                "Excellent performance. Continue the same "
                "study routine and participate actively in class."
            )
            risk_color = "#1976D2"
        elif predicted_score >= 50:
            risk_level = "Medium Risk"
            recommendation = (
                "Performance is satisfactory. Increase study "
                "hours and revise difficult topics regularly."
            )
            risk_color = "#D89D44"
        else:
